...
async def save_claim(*, user_id: int, username: str | None,
                     phone: str, amount: str, comment: str) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "INSERT INTO claims (tg_user_id, tg_username, phone, amount, comment) "
            "VALUES (?, ?, ?, ?, ?)",
...
async def notify_operator(bot: Bot, claim_id: int,
                          phone: str, amount: str, comment: str,
...
async def start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Привет! Если вода не пошла — оформи заявку на возврат.",
        reply_markup=kb,
    )
...
async def claim_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Отправь номер телефона кнопкой ниже или введи его текстом.",
...
async def got_contact(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Сколько списалось? Напиши сумму.",
...
async def got_amount(message: Message, state: FSMContext):
    ...
    await message.answer("Кратко опиши проблему.")
...
# -------------------------------------------------------------------
# Поддержка webhook-режима (для run_render.py)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Локальный polling (на всякий случай)
# -------------------------------------------------------------------
